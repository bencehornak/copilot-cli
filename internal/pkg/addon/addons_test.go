	...
	testCases := map[string]struct {
		...
		"return ErrAddonsNotFound if addons doesn't exist in a service": {
			...
			setupMocks: func(m addonMocks) {
				...
				m.ws.EXPECT().ListRegularFiles("mockPath").Return(nil, testErr)
			},
		...
		"return ErrAddonsNotFound if addons directory is empty in a service": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{}, nil)
			},
		...
		"return ErrAddonsNotFound if addons directory does not contain yaml files in a service": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"gitkeep"}, nil)
			},
			wantedErr: &ErrAddonsNotFound{
				ParentErr: nil,
			},
		...
		"ignore addons.parameters.yml files": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"addons.parameters.yml", "addons.parameters.yaml"}, nil)
			},
			wantedErr: &ErrAddonsNotFound{
				ParentErr: nil,
			},
		...
		"return err on invalid Metadata fields": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "invalid-metadata.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "invalid-metadata.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "invalid-metadata.yaml")).Return(second, nil)
			},
		...
		"returns err on invalid Parameters fields": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "invalid-parameters.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "invalid-parameters.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "invalid-parameters.yaml")).Return(second, nil)
			},
		...
		"returns err on invalid Mappings fields": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "invalid-mappings.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "invalid-mappings.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "invalid-mappings.yaml")).Return(second, nil)
			},
		...
		"returns err on invalid Conditions fields": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "invalid-conditions.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "invalid-conditions.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "invalid-conditions.yaml")).Return(second, nil)
			},
		...
		"returns err on invalid Resources fields": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "invalid-resources.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "invalid-resources.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "invalid-resources.yaml")).Return(second, nil)
			},
		...
		"returns err on invalid Outputs fields": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "invalid-outputs.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "invalid-outputs.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "invalid-outputs.yaml")).Return(second, nil)
			},
		...
		"merge fields successfully": {
			...
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath(testSvcName).Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"first.yaml", "second.yaml"}, nil)

				first, _ := os.ReadFile(filepath.Join("testdata", "merge", "first.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "first.yaml")).Return(first, nil)

				second, _ := os.ReadFile(filepath.Join("testdata", "merge", "second.yaml"))
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "second.yaml")).Return(second, nil)
			},
	...
	testCases := map[string]struct {
		...
		"returns ErrAddonsNotFound if there is no addons/ directory defined": {
			setupMocks: func(m addonMocks) {
				...
				m.ws.EXPECT().ListRegularFiles("mockPath").Return(nil, errors.New("some error"))
			},
			wantedErr: fmt.Errorf(`list addons under path mockPath: %w`, &ErrAddonsNotFound{
		...
		"returns empty string and nil if there are no parameter files under addons/": {
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath("api").Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"database.yaml"}, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "database.yaml")).Return([]byte(mockTemplate), nil)
			},
		},
		"returns an error if there are multiple parameter files defined under addons/": {
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath("api").Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"database.yml", "addons.parameters.yml", "addons.parameters.yaml"}, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "database.yml")).Return(nil, nil)
			},
		...
		"returns an error if cannot read parameter file under addons/": {
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath("api").Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"template.yml", "addons.parameters.yml"}, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "template.yml")).Return(nil, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "addons.parameters.yml")).Return(nil, errors.New("some error"))
			},
		...
		"returns an error if there are no 'Parameters' field defined in a parameters file": {
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath("api").Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"template.yaml", "addons.parameters.yml"}, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "template.yaml")).Return(nil, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "addons.parameters.yml")).Return([]byte(""), nil)
			},
		...
		"returns an error if reserved parameter fields is redefined in a parameters file": {
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath("api").Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"template.yaml", "addons.parameters.yml"}, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "template.yaml")).Return([]byte(mockTemplate), nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "addons.parameters.yml")).Return([]byte(`
Parameters:
  App: !Ref AppName
  Env: !Ref EnvName
...
`), nil)
		...
		"returns the content of Parameters on success": {
			setupMocks: func(m addonMocks) {
				m.ws.EXPECT().WorkloadAddonsPath("api").Return("mockPath")
				m.ws.EXPECT().ListRegularFiles("mockPath").Return([]string{"template.yaml", "addons.parameters.yaml"}, nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "template.yaml")).Return([]byte(`Parameters:
  App:
    Type: String
  Env:
    Type: String
  ...
  DiscoveryServiceArn:
    Type: String
`), nil)
				m.ws.EXPECT().ReadFile(filepath.Join("mockPath", "addons.parameters.yaml")).Return([]byte(`
Parameters:
  EventsQueue: 
    !Ref EventsQueue
